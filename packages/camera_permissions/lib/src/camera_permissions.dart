...
import 'package:camera_permissions/src/permission_enums.dart';
import 'package:meta/meta.dart';

class CameraPermissions {
  factory CameraPermissions() {
    if (_instance == null) {
      const MethodChannel methodChannel =
          MethodChannel('com.baseflow.flutter/camera_permissions');
      final EventChannel eventChannel = Platform.isAndroid
          ? const EventChannel(
              'com.baseflow.flutter/camera_permissions_events')
          : null;

      _instance = CameraPermissions.private(methodChannel, eventChannel);
    }
    return _instance;
  }

  @visibleForTesting
  CameraPermissions.private(this._methodChannel, this._eventChannel);

  static CameraPermissions _instance;
  ...
  ///
  /// Returns a [Future] containing the current permission status for the supplied [CameraPermissionLevel].
  Future<PermissionStatus> checkPermissionStatus(
      {CameraPermissionLevel level =
          CameraPermissionLevel.camera}) async {
    final int status =
  ...
  ///
  /// Returns a [Future] containing the current service status for the supplied [CameraPermissionLevel].
  ...
  Future<PermissionStatus> requestPermissions(
      {CameraPermissionLevel permissionLevel =
          CameraPermissionLevel.camera}) async {
