#import "CameraPermissionsPlugin.h"
#import "Enums.h"

@implementation CameraPermissionsPlugin {
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FlutterMethodChannel *channel =
      [FlutterMethodChannel methodChannelWithName:@"com.baseflow.flutter/camera_permissions"
                                  binaryMessenger:[registrar messenger]];
  CameraPermissionsPlugin *instance = [[CameraPermissionsPlugin alloc] initWithLocationManager];
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
  if ([@"checkPermissionStatus" isEqualToString:call.method]) {
    [CameraPermissionsPlugin checkPermissionStatus:call result:result];
  } else if ([@"checkServiceStatus" isEqualToString:call.method]) {
    [CameraPermissionsPlugin checkServiceStatus:result];
  } else if ([@"openAppSettings" isEqualToString:call.method]) {
    [CameraPermissionsPlugin openAppSettings:result];
...
+ (void)checkPermissionStatus:(FlutterMethodCall *)call result:(FlutterResult)result {
  PermissionLevel level = [CameraPermissionsPlugin decodePermissionLevel:call];
  PermissionStatus permissionStatus = [CameraPermissionsPlugin getPermissionStatus:level];
...
+ (PermissionStatus)getPermissionStatus:(PermissionLevel)permissionLevel {
  ...
  PermissionStatus status =
      [CameraPermissionsPlugin determinePermissionStatus:permissionLevel
...
- (void)requestPermission:(FlutterMethodCall *)call {
  ...
  PermissionStatus status = [CameraPermissionsPlugin getPermissionStatus:level];
  CLAuthorizationStatus authorizationStatus = [CLLocationManager authorizationStatus];
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  PermissionStatus permissionStatus =
      [CameraPermissionsPlugin determinePermissionStatus:_permissionLevel
